...
// NOTE: force-dynamic lives in ./layout.tsx — segment directives are
// ignored inside 'use client' modules, so it must stay server-side.
...
const docStr = (v: unknown): string => (typeof v === 'string' ? v : '');
const docStrArr = (v: unknown): string[] =>
  Array.isArray(v) ? v.filter((x): x is string => typeof x === 'string') : [];
const docObjArr = (v: unknown): DocRecord[] =>
  Array.isArray(v) ? v.filter((x): x is DocRecord => !!x && typeof x === 'object') : [];

/** DB project → form strings (arrays become editable text). */
const projectToForm = (p: DocRecord) => ({
    ...EMPTY_PROJECT_FORM,
    title: docStr(p.title),
    category: docStr(p.category) || 'Personal Project',
    projectType: p.projectType === 'small' ? 'small' : ('big' as 'big' | 'small'),
    description: docStr(p.description),
    image: docStr(p.image),
    codeUrl: docStr(p.codeUrl),
    liveUrl: docStr(p.liveUrl),
    tagline: docStr(p.tagline),
    videoUrl: docStr(p.videoUrl),
    stack: docStrArr(p.stack).length > 0 ? docStrArr(p.stack).join(', ') : docStr(p.stack),
    problemHeading: docStr(p.problemHeading),
    problem: docStrArr(p.problem).join('\n\n'),
    constraints: docStrArr(p.constraints).join('\n'),
    processHeading: docStr(p.processHeading),
    processIntro: docStr(p.processIntro),
    phases: docObjArr(p.phases).map((ph) => `${docStr(ph.label)} | ${docStr(ph.title)} | ${docStr(ph.desc)}${ph.marker ? ' | !' : ''}`).join('\n'),
    decisions: docObjArr(p.decisions).map((d) => `${docStr(d.decision)} | ${docStr(d.rejected)} | ${docStr(d.reason)}`).join('\n'),
    outcomeHeading: docStr(p.outcomeHeading),
    outcomeBody: docStrArr(p.outcomeBody).join('\n\n'),
    role: docStr(p.role),
    rolePoints: docStrArr(p.rolePoints).join('\n'),
    reflectionHeading: docStr(p.reflectionHeading),
    reflection: docStr(p.reflection),
    stats: docObjArr(p.stats).map((s) => `${docStr(s.value)} | ${docStr(s.label)}`).join('\n'),
});

/** Form strings → API payload (text becomes arrays). */
const formToPayload = (f: typeof EMPTY_PROJECT_FORM) => ({
    title: f.title,
    category: f.category,
    ...
    problemHeading: f.problemHeading.trim(),
    problem: splitParas(f.problem),
    constraints: splitLines(f.constraints),
    processHeading: f.processHeading.trim(),
    processIntro: f.processIntro.trim(),
    phases: splitLines(f.phases).map((line) => {
        const [label = '', title = '', desc = '', flag = ''] = line.split('|').map((s) => s.trim());
        return { label, title, desc, marker: flag === '!' };
    }),
    decisions: splitLines(f.decisions).map((line) => {
        const [decision = '', rejected = '', reason = ''] = line.split('|').map((s) => s.trim());
        return { decision, rejected, reason };
    }).filter((d) => d.decision),
    outcomeHeading: f.outcomeHeading.trim(),
    outcomeBody: splitParas(f.outcomeBody),
    role: f.role.trim(),
    rolePoints: splitLines(f.rolePoints),
    reflectionHeading: f.reflectionHeading.trim(),
    reflection: f.reflection.trim(),
    stats: splitLines(f.stats).map((line) => {
        const [value = '', label = ''] = line.split('|').map((s) => s.trim());
        return { value, label };
    }).filter((s) => s.value || s.label),
});

const projectHasStory = (p: DocRecord) =>
    !!(docStr(p.tagline) || docStr(p.videoUrl) || docStrArr(p.problem).length || docObjArr(p.decisions).length || docStrArr(p.outcomeBody).length);
...
export default function AdminDashboard() {
    ...
    const handleAddProject = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const payload = formToPayload(newProject);
            const res = await fetch('/api/projects', {
                method: isEdit ? 'PUT' : 'POST',
                headers: { 'Content-Type': 'application/json', 'x-admin-key': adminKey },
                body: JSON.stringify(isEdit ? { ...payload, _id: editingProject._id } : payload),
            });
            const data = await res.json();
            if (!res.ok) throw new Error(data.error || (isEdit ? 'Failed to update project' : 'Failed to add project'));
            setProjects((prev) => {
                const updated = isEdit
                    ? prev.map((p) => (p._id === editingProject._id ? { ...p, ...payload } : p))
                    : [data.project, ...prev];
    ...
    if (!isAuthenticated) {
        return (
            <div className={`admin-shell ${isDarkTheme ? 'admin-dark' : 'admin-light'} min-h-screen flex items-center justify-center px-4`}>
                <div className="w-full max-w-md">
                    ...
                    <div className="text-center mb-10">
                        ...
                        <p className="admin-kicker">Portfolio operations / secure access</p>
    ...
    // Dashboard
    return (
        <div className={`admin-shell ${isDarkTheme ? 'admin-dark' : 'admin-light'} min-h-screen text-white font-sans`}>
            {/* Header */}
            <header className="sticky top-0 z-50 admin-header backdrop-blur-xl">
                <div className="max-w-[1500px] mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <div className="admin-brand-mark w-9 h-9 rounded-xl flex items-center justify-center">
                    ...
                    <div className="flex items-center gap-2 sm:gap-3">
                        ...
                        <button
                            ...
                            className="admin-logout text-zinc-500 hover:text-white text-sm transition-colors flex items-center gap-2"
                        >
            ...
            <main className="max-w-[1500px] mx-auto px-4 sm:px-6 py-6 sm:py-8">
                ...
                {/* Tab Navigation */}
                <div className="admin-tabs flex flex-wrap gap-1 p-1 bg-zinc-900/50 rounded-2xl border border-zinc-800/50 mb-8 w-full lg:w-fit">
                    <button
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                        ...
                                        >
                                            ...
                                            <div className="relative w-full h-32 bg-black/40 flex items-center justify-center overflow-hidden">
                                                ...
                                                <div className="absolute top-2 right-2 flex flex-col items-end gap-1">
                                                    ...
                                                    {projectHasStory(project) && (
                                                        <span className="px-2 py-0.5 text-[8px] font-extrabold rounded-full uppercase tracking-wider bg-yellow-400/15 text-yellow-400 border border-yellow-400/30">
                                                            Story ✓
                                                        </span>
                                                    )}
                                                    {project.videoUrl && (
                                                        <span className="px-2 py-0.5 text-[8px] font-extrabold rounded-full uppercase tracking-wider bg-blue-500/15 text-blue-400 border border-blue-500/30">
                                                            Video ✓
                                                        </span>
                                                    )}
                                                </div>
                                            ...
                                            <div className="p-4 flex-1 flex flex-col justify-between">
                                                ...
                                                <div className="mt-4 pt-3 border-t border-zinc-800/50 flex items-center justify-between gap-3">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <button
                                                            onClick={() => { setEditingProject(project); setNewProject(projectToForm(project)); }}
                        ...
                        <div className="bg-zinc-900/50 border border-zinc-800/50 rounded-xl p-6 h-fit space-y-6">
                            <div className="flex items-center justify-between">
                                ...
                                {editingProject && <button onClick={() => { setEditingProject(null); setNewProject({ ...EMPTY_PROJECT_FORM }); }} className="text-xs text-zinc-500 hover:text-white transition-colors cursor-pointer">✕ Cancel Edit</button>}
                            ...
                            <form onSubmit={handleAddProject} className="space-y-4 text-sm">
                                ...
                                <details className="rounded-lg border border-zinc-800 bg-zinc-950/60 overflow-hidden">
                                    <summary className="px-4 py-3 text-xs font-bold tracking-wider uppercase text-zinc-300 hover:text-yellow-400 transition-colors cursor-pointer select-none flex items-center justify-between">
                                        <span>📖 Project Story {projectHasStory(formToPayload(newProject)) ? <span className="text-yellow-400">(filled ✓)</span> : <span className="text-zinc-600">(optional)</span>}</span>
                                        <span className="text-zinc-600">▾</span>
                                    </summary>
                                    <div className="px-4 pb-4 pt-1 space-y-4 border-t border-zinc-800/70">
                                        <div>
                                            <label className="block text-zinc-400 text-xs font-medium mb-1.5">One-line outcome (headline, read first)</label>
                                            <input
                                                type="text"
                                                value={newProject.tagline || ''}
                                                onChange={(e) => setNewProject({ ...newProject, tagline: e.target.value })}
                                                placeholder="e.g. A live hostel-ops platform where nothing disappears into a register."
                                                className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4.5 py-2.5 text-white placeholder:text-zinc-700 outline-none focus:border-yellow-400 transition-colors"
                                            />
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 gap-4">
                                            <div>
                                                <label className="block text-zinc-400 text-xs font-medium mb-1.5">Demo video URL (YouTube link or direct .mp4)</label>
                                                <input
                                                    type="text"
                                            ...
                                            <div>
                                                <label className="block text-zinc-400 text-xs font-medium mb-1.5">Tech stack (comma separated)</label>
                                                <input
                                                    type="text"
                                        ...
                                        <div>
                                            <label className="block text-zinc-400 text-xs font-medium mb-1.5">Problem heading (make it carry the story)</label>
                                            <input
                                                type="text"
                                                value={newProject.problemHeading || ''}
                                                onChange={(e) => setNewProject({ ...newProject, problemHeading: e.target.value })}
                                                placeholder="e.g. Hostel complaints went into a register — and never came back out."
                                                className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4.5 py-2.5 text-white placeholder:text-zinc-700 outline-none focus:border-yellow-400 transition-colors"
                                            />
                                        </div>
                                        <div>
                                            <label className="block text-zinc-400 text-xs font-medium mb-1.5">Problem (paragraphs — separate with a blank line)</label>
                                            <textarea
                                                rows={4}
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                            <div>
                                                <label className="block text-zinc-400 text-xs font-medium mb-1.5">Process heading</label>
                                                <input
                                                    type="text"
                                                    value={newProject.processHeading || ''}
                                                    onChange={(e) => setNewProject({ ...newProject, processHeading: e.target.value })}
                                                    placeholder="How it was built."
                                                    className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4.5 py-2.5 text-white placeholder:text-zinc-700 outline-none focus:border-yellow-400 transition-colors"
                                                />
                                            </div>
                                            <div>
                                                <label className="block text-zinc-400 text-xs font-medium mb-1.5">Process intro (optional)</label>
                                                <input
                                                    type="text"
                                                    value={newProject.processIntro || ''}
                                                    onChange={(e) => setNewProject({ ...newProject, processIntro: e.target.value })}
                                                    placeholder="One-line intro under the heading"
                                                    className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4.5 py-2.5 text-white placeholder:text-zinc-700 outline-none focus:border-yellow-400 transition-colors"
                                                />
                                            </div>
                                        </div>
                                        <div>
                                            <label className="block text-zinc-400 text-xs font-medium mb-1.5">Timeline (one per line: label | title | what happened — trailing ! marks a milestone)</label>
                                            <textarea
                                                rows={4}
                                                value={newProject.phases || ''}
                                                onChange={(e) => setNewProject({ ...newProject, phases: e.target.value })}
                                                placeholder={'Month 1 | Idea and scope | Mapped out what the build could mean\nMonth 3 | Official deadline | The build kept going anyway | !'}
                                                className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4.5 py-2.5 text-white placeholder:text-zinc-700 outline-none focus:border-yellow-400 transition-colors resize-none font-mono text-xs"
                                            />
                                        </div>

                                        <div>
                                            <label className="block text-zinc-400 text-xs font-medium mb-1.5">Decision log (one per line: decision | what you rejected | why)</label>
                                            <textarea
                                                rows={4}
                                                value={newProject.decisions || ''}
                                                onChange={(e) => setNewProject({ ...newProject, decisions: e.target.value })}
                                                placeholder={'Rebuilt wiring from scratch | Keep patching v1 | Every fix exposed the next weak joint'}
                                                className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4.5 py-2.5 text-white placeholder:text-zinc-700 outline-none focus:border-yellow-400 transition-colors resize-none font-mono text-xs"
                                            />
                                        </div>

                                        <div>
                                            <label className="block text-zinc-400 text-xs font-medium mb-1.5">Outcome heading</label>
                                            <input
                                                type="text"
                                                value={newProject.outcomeHeading || ''}
                                                onChange={(e) => setNewProject({ ...newProject, outcomeHeading: e.target.value })}
                                                placeholder="What shipped."
                                                className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4.5 py-2.5 text-white placeholder:text-zinc-700 outline-none focus:border-yellow-400 transition-colors"
                                            />
                                        </div>
                                        <div>
                                            <label className="block text-zinc-400 text-xs font-medium mb-1.5">Outcome (paragraphs — real numbers beat adjectives)</label>
                                            <textarea
                                                rows={3}
                                                value={newProject.outcomeBody || ''}
                                                onChange={(e) => setNewProject({ ...newProject, outcomeBody: e.target.value })}
                                        ...
                                        <div>
                                            <label className="block text-zinc-400 text-xs font-medium mb-1.5">Your role (one line)</label>
                                            <input
                                                type="text"
                                        ...
                                        <div>
                                            <label className="block text-zinc-400 text-xs font-medium mb-1.5">Role points (one per line)</label>
                                            <textarea
                                                rows={3}
                                                value={newProject.rolePoints || ''}
                                                onChange={(e) => setNewProject({ ...newProject, rolePoints: e.target.value })}
                                                placeholder={'Designed the complaint lifecycle\nBuilt the platform end to end'}
                                                className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4.5 py-2.5 text-white placeholder:text-zinc-700 outline-none focus:border-yellow-400 transition-colors resize-none"
                                            />
                                        </div>

                                        <div>
                                            <label className="block text-zinc-400 text-xs font-medium mb-1.5">Reflection heading</label>
                                            <input
                                                type="text"
                                                value={newProject.reflectionHeading || ''}
                                                onChange={(e) => setNewProject({ ...newProject, reflectionHeading: e.target.value })}
                                                placeholder="What I'd do differently."
                                                className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4.5 py-2.5 text-white placeholder:text-zinc-700 outline-none focus:border-yellow-400 transition-colors"
                                            />
                                        </div>
                                        <div>
                                            <label className="block text-zinc-400 text-xs font-medium mb-1.5">Reflection (honest {'>'} generic)</label>
                                            <textarea
                                                rows={3}
                                        ...
                                        <div>
                                            <label className="block text-zinc-400 text-xs font-medium mb-1.5">Stat strip (one per line: value | label)</label>
                                            <textarea
                                                rows={3}
...
function ThemeToggle({ isDark, onToggle }: { isDark: boolean; onToggle: () => void }) {
    return (
        ...
        >
            <span className={`admin-theme-icon ${isDark ? 'is-active' : ''}`} aria-hidden="true">◐</span>
